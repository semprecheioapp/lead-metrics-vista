-- Phase 1: Database Structure - Fixed migrations for agent management system
...
-- Create papeis_empresa table (roles for companies) - Fixed with correct FK type
CREATE TABLE IF NOT EXISTS public.papeis_empresa (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id bigint REFERENCES public.empresas(id) ON DELETE CASCADE,
  name text NOT NULL,
  description text,
...
-- Create membros_empresa table (company members) - Fixed with correct FK types
...
-- Create convites_empresa table (company invites) - Fixed with correct FK types
...
-- Create audit_logs table - Fixed with correct FK types
...
-- Create helper functions - Fixed with correct parameter types
